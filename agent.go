...
// Agent wraps a composition function as a Node[I, O].
...
// like any other node.
//
//	agent := reflow.Agent[Request, Response]("support",
...
//	)
//
//	out, err := reflow.Run(ctx, agent, envelope)
func Agent[I, O any](name string, fn func(context.Context, *Steps, Envelope[I]) Envelope[O]) Node[I, O] {
	return &agentNode[I, O]{name: name, fn: fn}
}
...
func (a *agentNode[I, O]) Resolve(_ context.Context, in Envelope[I]) (Envelope[I], error) {
	return in, nil
}

func (a *agentNode[I, O]) Act(ctx context.Context, in Envelope[I]) (Envelope[O], error) {
	var s Steps
	out := a.fn(ctx, &s, in)
	if s.Err() != nil {
		return out, s.Err()
...
func (a *agentNode[I, O]) Settle(_ context.Context, _ Envelope[I], out Envelope[O], actErr error) (Envelope[O], bool, error) {
	if actErr != nil {
		// Return done=false with nil error so WithRetry can retry.
		// The actErr is settled as a hint for the next iteration.
		out = out.WithHint("agent.error", actErr.Error(), "")
